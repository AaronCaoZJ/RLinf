...
class PickAndPlaceMultiViewEnv(PickAndPlaceEnv):
    ...
    def _default_sensor_configs(self):
        ...
        # Table-side cameras: directly left/right of table center line
        table_side_dist = 1.0 * float(self.DISTANCE_SCALE)
        side_height = self.TABLE_Z + 0.30
        scaled_poses["table_left"] = [self._TABLE_CENTER_X, table_side_dist, side_height]
        scaled_poses["table_right"] = [self._TABLE_CENTER_X, -table_side_dist, side_height]

        table_side_target = np.array([self._TABLE_CENTER_X, 0.0, self.TABLE_Z + 0.08])

        configs = []
        for name, eye in scaled_poses.items():
            look_at = table_side_target if name in {"table_left", "table_right"} else target
            configs.append(
                CameraConfig(
                    uid=f"cam_{name}",
                    pose=make_look_at_pose(eye=eye, target=look_at),
                    width=640,
                    height=480,
                    near=0.01,
                    far=20,
...
@register_env("BlockPAP-SideView-v1", max_episode_steps=200)
class PickAndPlaceSideViewEnv(PickAndPlaceEnv):
    """Base env + one left-side sensor camera for replay concatenation."""
    SIDE_CAM_NAME = "cam_side_left"

    @property
    def _default_sensor_configs(self):
        eye = np.array([self._TABLE_CENTER_X, 1.2, self.TABLE_Z + 0.30])
        tgt = np.array([self._TABLE_CENTER_X, 0.0, self.TABLE_Z + 0.08])
        return [
            CameraConfig(
                uid=self.SIDE_CAM_NAME,
                pose=make_look_at_pose(eye=eye, target=tgt),
...
def main():
    ...
    if len(ordered) == 6:
        ...
        bottom_row = np.concatenate(
            [rendered["cam_top"], rendered["cam_table_left"], rendered["cam_table_right"]], axis=1
        )
